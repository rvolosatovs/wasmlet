...
pub fn add_to_linker<T: WasiFilesystemView>(linker: &mut Linker<T>) -> wasmtime::Result<()> {
    let closure = annotate_filesystem(|cx| WasiFilesystemImpl(cx));
    crate::engine::bindings::wasi::filesystem::types::add_to_linker_get_host(linker, closure)?;
    crate::engine::bindings::wasi::filesystem::preopens::add_to_linker_get_host(linker, closure)?;
    Ok(())
}
