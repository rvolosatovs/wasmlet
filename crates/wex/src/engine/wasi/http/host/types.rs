...
use std::sync::Arc;
use std::{io::Cursor, sync::TryLockError};
...
use crate::engine::wasi::http::{
    host::{
        ...
        push_fields, push_fields_child, push_request, push_response,
    },
    OutgoingBodyContentSender, OutgoingBodySender,
};
use crate::engine::wasi::http::{
    Body, BodyContext, BodyFrame, ContentLength, FutureIncomingResponse, FutureTrailers,
    IncomingBody, IncomingRequest, IncomingResponse, OutgoingBody, OutgoingRequest,
    OutgoingResponse, Request, RequestOptions, Response, ResponseOutparam, WasiHttpImpl,
    WasiHttpView, DEFAULT_BUFFER_CAPACITY,
};
use crate::engine::wasi::io::{InputStream, OutputStream, Pollable};
use crate::engine::{
    bindings::wasi::clocks::monotonic_clock::Duration, wasi::io::ChannelInputStream,
};
use crate::engine::{
    bindings::wasi::http::types::{
        ErrorCode, FieldName, FieldValue, HeaderError, Host, HostFields,
        HostFutureIncomingResponse, HostFutureTrailers, HostIncomingBody, HostIncomingRequest,
        HostIncomingResponse, HostOutgoingBody, HostOutgoingRequest, HostOutgoingResponse,
        HostRequestOptions, HostResponseOutparam, Method, Scheme, StatusCode, Trailers,
    },
    wasi::io::HttpInputStream,
};
use crate::engine::{wasi, ResourceView as _, WithChildren};

use super::{delete_body, get_body, get_body_mut};
    ...
    T: WasiHttpView,
{
    fn write(
        ...
        let body = get_body(self.table(), &body)?;
    ...
    fn finish(
        ...
        let trailers = trailers
            ...
            .transpose()?;
        let body = delete_body(self.table(), body)?;
        let mut body = match body.try_lock() {
            Ok(body) => body,
            Err(TryLockError::WouldBlock) => bail!("outgoing body lock contended"),
            Err(TryLockError::Poisoned(..)) => bail!("outgoing body lock poisoned"),
    ...
    T: WasiHttpView,
{
    fn consume(
        ...
        let IncomingRequest { body, .. } = get_request_mut(self.table(), &req)?;
    ...
    fn method(&mut self, req: Resource<IncomingRequest>) -> wasmtime::Result<Method> {
        let IncomingRequest { method, .. } = get_request(self.table(), &req)?;
        Ok(method.into())
    }
    ...
    fn scheme(&mut self, req: Resource<IncomingRequest>) -> wasmtime::Result<Option<Scheme>> {
        let IncomingRequest { scheme, .. } = get_request(self.table(), &req)?;
        Ok(scheme.as_ref().map(Into::into))
    }

    fn authority(&mut self, req: Resource<IncomingRequest>) -> wasmtime::Result<Option<String>> {
        let IncomingRequest { authority, .. } = get_request(self.table(), &req)?;
        Ok(authority.as_ref().map(|auth| auth.as_str().into()))
    }
    ...
    T: WasiHttpView,
{
    ...
    fn body(
        ...
        let res @ OutgoingResponse { body: None, .. } = get_response_mut(self.table(), &res)?
        else {
            return Ok(Err(()));
        };
