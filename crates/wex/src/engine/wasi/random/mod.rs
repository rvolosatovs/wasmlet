...
pub fn add_to_linker<T: WasiRandomView>(linker: &mut Linker<T>) -> wasmtime::Result<()> {
    let closure = annotate_random(|cx| WasiRandomImpl(cx));
    crate::engine::bindings::wasi::random::random::add_to_linker_get_host(linker, closure)?;
    crate::engine::bindings::wasi::random::insecure::add_to_linker_get_host(linker, closure)?;
    crate::engine::bindings::wasi::random::insecure_seed::add_to_linker_get_host(linker, closure)?;
    Ok(())
}
